...
| A package shows `rc 2` in the report and its device stays unknown (seen: NVIDIA on the 7620) | `Dell\Reports\<TAG>-pnputil.log`: "cannot find the file specified" | the package was extracted incompletely; on the Surface re-extract it with 7-Zip and run `.\build_restorer.ps1 -ValidateDrivers -ExtractOnly` until every package stages, then rebuild the sticks |
