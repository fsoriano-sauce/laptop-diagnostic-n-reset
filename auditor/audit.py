...
def mount_usb_rw() -> str:
    """
    Mount (or remount) the boot USB partition as read/write to USB_MOUNT_POINT.
    Returns the path to the writable directory, or '' on failure.
    """
    partition = find_boot_usb_partition()
    if not partition:
        print("[!] WARNING: Could not detect boot USB partition.")
    ...
    # Try remount first (if already mounted read-only)
...
def sync_and_unmount():
    """Flush writes and cleanly unmount the USB data partition."""
    os.system("sync")
    os.system(f"umount {USB_MOUNT_POINT} 2>/dev/null")
...
def offer_wipe(data: dict):
    ...
    print("  ╚══════════════════════════════════════════╝")
    answer2 = input("  Type CONFIRM to proceed > ").strip()
    if answer2 != "CONFIRM":
        print("  Wipe cancelled.")
        return
    ...
    print(f"\n  Wiping {disk}...")
    if "nvme" in disk:
        ret = os.system(f"blkdiscard {disk}")
    else:
